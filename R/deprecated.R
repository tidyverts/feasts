...
default_time_units <- function(...){
  if(packageVersion("tsibble") >= "0.8.9.9000") tsibble::default_time_units(...) else tsibble::time_unit(...)
}

units_since <- function(...){
  if(packageVersion("tsibble") >= "0.8.9.9000") as.double(...) else tsibble::units_since(...)
}
