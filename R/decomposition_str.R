specials_str <- fabletools::new_specials(
...
estimate_str <- function(y, trend.args, season.args, lowpass.args,
                         iterations = 2, ...){
  if(any(is.na(y))){
...
train_str <- function(.data, formula, specials, iterations = 2, ...){
  ...
  structure(
    ...
    class = "str_decomposition"
  )
}

#' @importFrom fabletools components as_dable
#' @export
components.str_decomposition <- function(object, ...){
...
#' @export
fitted.str_decomposition <- function(object, ...) {
  object[["decomposition"]][[object[["response"]]]] - residuals(object)
}

#' @importFrom stats residuals
#' @export
residuals.str_decomposition <- function(object, ...) {
...
#'
#' @export
generate.str_decomposition <- function(x, new_data, specials = NULL, ...){
...
#' @export
model_sum.str_decomposition <- function(x){
  "STR"
}
...
#' @export
STR <- function(formula, iterations = 2, robust = FALSE, ...){
  dcmp <- new_model_class("STR",
                          train = train_str, specials = specials_str,
